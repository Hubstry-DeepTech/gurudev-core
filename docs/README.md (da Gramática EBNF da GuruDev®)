...
* O lexer reconhece todos os tokens definidos nesta EBNF, incluindo `FOREIGN_CODE_CONTENT` para blocos de código estrangeiro.
...
* **WHITESPACE** e **NEWLINE** são tokens produzidos pelo lexer, mas geralmente ignorados pelo parser, exceto onde explicitamente referenciados.
* **Comentários** são ignorados pelo lexer e não chegam ao parser.
* **A gramática é auto-contida:** Todos os terminais estão definidos ao final do arquivo.
* **Consistência:** Os nomes dos tokens na EBNF batem exatamente com os nomes do Enum do lexer.

---

## 🚀 Contribuindo

* Sugestões, correções e expansões são bem-vindas!
* Para contribuir, abra um Pull Request ou Issue neste repositório.
